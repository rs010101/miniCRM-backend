...
export const sendCampaignMessages = async (userId, campaignId) => {
  try {
    ...
    // If campaign has specific customerIds, use those
    if (campaign.customerIds && campaign.customerIds.length > 0) {
      customers = await Customer.find({
        _id: { $in: campaign.customerIds },
        userId: userId
      });
    } else {
      // Otherwise, get all customers in the segment
      customers = await getCustomersForSegment(userId, campaign.segmentRuleId);
    }
